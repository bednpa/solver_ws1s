...
public:
  ...
  void dump() = 0; 
  virtual void genCode(Nfa &in_aut, unsigned long act_ident_count, remapVars mapping, int carry = -1) = 0;
  virtual void getDepth(int *d) = 0; 
};
...
public:
  ...
  void getDepth(int *d) { (*d)++; T->getDepth(d); }
...
public:
  ...
  void freeVars(IdentList*, IdentList*);
  void getDepth(int *d) { (*d)++; t1->getDepth(d); t2->getDepth(d); }
...
public:
  ...
  void freeVars(IdentList*, IdentList*);
  void getDepth(int *d) { *d = *d + 2; T1->getDepth(d); T2->getDepth(d); }
...
public:
  ...
  void genCode(Nfa &in_aut, unsigned long act_ident_count, remapVars mapping, int carry = -1);
...
public:
  ...
  void genCode(Nfa &in_aut, unsigned long act_ident_count, remapVars mapping, int carry = -1);
  void getDepth(int *d) { return; } // TODO tady 
  void dump();
...
public:
  remapVars() {}
  void addRecord(Ident origin_ident, Ident new_ident);
  Ident getMapping(const Ident &var);
  Ident getRevMapping(const Ident &var);
};
