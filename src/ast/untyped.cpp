...
extern int options_mode ;
extern bool options_m2l;
extern bool options_alternativeM2LStr;
...
UntypedExp_Set::genAST()
{
  ...
  if (options_mode == TREE && expList->empty())
...
Type_Declaration::genAST(MonaAST &monaAST)
{
  ...
  //symbolTable.setTypeNumber(id, initTreetype(name->str, v->size())); !!!! puvodne to bylo odkomentovano
...
/*#warning bugfix: 'where true'*/
  ...
  //if (numTypes > 0)
    //initTreetypes(numTypes); !!!!! puvodne bylo odkomentovano
  ...
  // make GTA guide
  if (options_mode == TREE) !!!!! puvodne bylo odkomentovano
    makeGTAGuide();*/
