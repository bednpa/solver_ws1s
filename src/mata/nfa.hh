...
struct State // new definition of state with added jump/skip information
{
    unsigned long state;
    unsigned long jump;

    State() { jump = 1; }
    State(unsigned long s) { state = s; jump = 1; }
    State(unsigned long s, unsigned long j) { state = s; jump = j; }

    operator unsigned long() const { return state; }
    State operator++() { return ++state; }
    State operator=(State rhs) { state = rhs.state, jump = rhs.jump; return *this; }
};
...
/*static constexpr struct Limits {
    State maxState = ULLONG_MAX; //std::numeric_limits<State>::max();
    State minState = 0; //std::numeric_limits<State>::min();
...
constexpr Symbol EPSILON = ULLONG_MAX; //limits.maxSymbol; EDITED
 ...
 */
Nfa intersection(const Nfa& const_lhs, const Nfa& const_rhs,
...
template <>
struct hash<Mata::Nfa::State>
{
    inline size_t operator()(const Mata::Nfa::State& s) const
    {
        std::hash<unsigned long> hash_fn;
        return hash_fn(s.state);
    }
};
