    ...
    for (State sourceState{ 0 }; sourceState < num_of_states; ++sourceState) {
        for (const Move &transition: aut.transition_relation[sourceState]) {
            for (const State targetState: transition.states_to) {
                ...
                if (targetState.jump == 1)
                {
                    result.add_trans(targetState, transition.symbol, State{sourceState, 1});
                }
                else
                {
                    State inner_state;
                    inner_state.state = result.add_state();
                    inner_state.jump = targetState.jump - 1;
                    result.add_trans(targetState, 0, State{inner_state, targetState.jump - 1}); 
                    result.add_trans(targetState, 1, State{inner_state, targetState.jump - 1});
                    result.add_trans(inner_state, transition.symbol, State{sourceState, 1});
                }
            }
        }
    }
    ...
    for (State s = 0; s != transition_relation.size(); ++s) {
        for (const Move &t: transition_relation[s]) {
            ...
            for (State sTo: t.states_to) {
                outputStream << s << " -> {" << sTo << " } [label=\"" << t.symbol << "/" << sTo.jump << "\"];" << std::endl;
            }
            //outputStream << "} [label=\"" << t.symbol << "/" << t.states_to.ToVector()[0].jump << "\"];" << std::endl;
        }
    }
...
Nfa Mata::Nfa::uni(const Nfa &lhs, const Nfa &rhs) {
    // pridavam levy automat do praveho automatu
    Nfa unionAutomaton = rhs;

    // mapuju stav leveho automatu na jeho odpovidajici novy stav v pravem automatu
    ...
    // puvodni pocatecni stavy leveho autmatu nastavim jako pocateni v pravem automatu
    for (State thisInitialState : lhs.initial_states) {
        unionAutomaton.make_initial(thisStateToUnionState[thisInitialState]);
    }

    // puvodni koncove stavy leveho autmatu nastavim jako pocateni v pravem automatu
    ...
    for (State thisState = 0; thisState < lhs.transition_relation.size(); ++thisState) {
        ...
        for (const Move &transitionFromThisState : lhs.transition_relation[thisState]) {
            ...
            for (State stateTo : transitionFromThisState.states_to) {
                transitionFromUnionState.states_to.insert(State{thisStateToUnionState[stateTo].state, stateTo.jump});
            }
        ...
        std::unordered_map<StateSet, State> *subset_map)
{
    ...
    Nfa aut_copy = aut;
    std::vector<std::pair<State,State>> end_inner_states;
    ...
    while (!worklist.empty()) {
        ...
        Moves act_moves[S.size()];
        int i = 0;
        for (State q: S) {
            act_moves[i] = aut_copy.transition_relation[q];
            i++;
        }
        i = 0;
        for (State q: S) {
            Mata::Util::push_back(synchronized_iterator, act_moves[i]);
            i++;
        }
        ...
        while (synchronized_iterator.advance()) {
            ...
            std::vector<State> old_T; 

            for (auto m: moves){
                for (auto s: m->states_to) {
                    old_T.push_back(s); 
                }
            }

            unsigned long min_t_jump = std::min_element(old_T.begin(), old_T.end(), [](const auto &e1, const auto &e2) {return e1.jump < e2.jump; })->jump;
            StateSet T;

            for (const auto &t: old_T) {
                if (min_t_jump != t.jump) {
                    for (const auto &s: S) {
                        if (aut_copy.has_trans(s, currentSymbol, State{t.state, t.jump})) {

                            // remove A --> B
                            aut_copy.remove_trans(s, currentSymbol, State{t.state, t.jump});

                            // create inner state
                            State inner_state;
                            auto it = std::find_if(end_inner_states.begin(), end_inner_states.end(), [&t,&min_t_jump](const auto &pair) { return pair.first.state == t.state && pair.first.jump - pair.second.jump == t.jump - min_t_jump && t.jump - min_t_jump > 0; });

                            if (it != end_inner_states.end()) {
                                inner_state.state = it->second.state;
                                inner_state.jump = min_t_jump;
                            } else {
                                inner_state.state = aut_copy.add_state();
                                inner_state.jump = min_t_jump;
                                isFinal.insert(isFinal.begin() + inner_state, false);
                                end_inner_states.push_back(std::pair(t, inner_state));
                                // add X --> B
                                aut_copy.add_trans(inner_state, 0, State{t.state, t.jump - min_t_jump});
                                aut_copy.add_trans(inner_state, 1, State{t.state, t.jump - min_t_jump});
                            }

                            // set B = X
                            T.insert(State{inner_state, min_t_jump});
                            // add A --> X
                            aut_copy.add_trans(s, currentSymbol, State{inner_state, min_t_jump});
                        }
                    }
                } else {
                    T.insert(State{t.state, min_t_jump});
                }
            }
...
/*std::ostream& std::operator<<(std::ostream& os, const Mata::Nfa::Nfa& nfa) { // because of compiler warning
